  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <Link href="/units" className="inline-flex items-center gap-2 rounded-full border border-slate-300 bg-white/80 px-4 py-2 text-sm font-bold text-slate-700 backdrop-blur transition hover:border-sky-300 hover:text-sky-700 dark:border-slate-700 dark:bg-slate-900/80 dark:text-slate-100 dark:hover:border-sky-800 dark:hover:text-sky-400">
          عرض الجميع
            ...
            <Link key={unit.id} href={`/units/${unit.slug}`} className={`premium-card group relative overflow-hidden rounded-[1.8rem] p-5 transition hover:-translate-y-1 ${palette.border}`}>
              ...
              <div className="relative">
                ...
                <div className="mt-5 grid grid-cols-3 gap-2 text-xs text-slate-500 dark:text-slate-400">
                  <div className="rounded-2xl bg-white/70 px-3 py-2 text-center dark:bg-slate-800/70">
