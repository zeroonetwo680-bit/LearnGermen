...
export function FeaturedLessonsSection({ lessons }: { lessons: LessonSummaryDto[] }) {
  return (
    <section className="animate-fade-up animate-fade-up-delay-2 space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <Link href="/lessons" className="button-shine pressable inline-flex items-center gap-2 rounded-full border border-slate-300 bg-white/80 px-4 py-2 text-sm font-bold text-slate-700 backdrop-blur transition hover:border-sky-300 hover:text-sky-700 dark:border-slate-700 dark:bg-slate-900/80 dark:text-slate-100 dark:hover:border-sky-800 dark:hover:text-sky-400">
          كل الدروس
