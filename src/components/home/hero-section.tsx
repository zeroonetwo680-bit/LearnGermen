...
export function HeroSection({ content }: { content: HomeContentDto }) {
  return (
    <section className="relative overflow-hidden rounded-[2.25rem] bg-[linear-gradient(135deg,#082f49_0%,#0f4c81_42%,#0891b2_100%)] px-6 py-10 text-white shadow-[0_30px_80px_rgba(8,47,73,0.35)] sm:px-10 sm:py-14">
      ...
      <div className="relative grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
        <div>
          <span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-4 py-2 text-sm font-semibold text-sky-50 backdrop-blur">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link href="/lessons" className="inline-flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-bold text-sky-900 transition hover:-translate-y-0.5 hover:bg-sky-50">
              {content.primaryCta}
              <ArrowUpLeft className="size-4" />
            </Link>
            <Link href="/units" className="rounded-full border border-white/25 bg-white/5 px-5 py-3 text-sm font-bold text-white transition hover:bg-white/10">
        ...
        <div className="premium-card rounded-[1.75rem] border-white/10 bg-white/10 p-5 text-white dark:border-white/10 dark:bg-white/10">
          ...
          <div className="mt-6 space-y-3">
            {content.features.slice(0, 4).map((feature, index) => (
              <div key={feature.id} className="rounded-2xl border border-white/10 bg-white/10 p-4 backdrop-blur-sm">
